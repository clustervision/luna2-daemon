...
if [ "$MAKE_BOOT" == "yes" ]; then
    ...
    CHROOT_INITRAMFS=$(chroot "$rootmnt" /bin/bash -c 'if command -v dracut >/dev/null 2>&1; then echo dracut; elif command -v update-initramfs >/dev/null 2>&1; then echo update-initramfs; fi')
    if [ "$CHROOT_INITRAMFS" == "dracut" ]; then
        chroot "$rootmnt" /bin/bash -c "dracut -f --mdadmconf --add=mdraid --add-drivers='raid1'"
    elif [ "$CHROOT_INITRAMFS" == "update-initramfs" ]; then
        chroot "$rootmnt" /bin/bash -c "update-initramfs -u"
    fi

    echo "*** RAID1 script: configuring grub/shim boot partition"
    GRUB_CMDLINE_LINUX=$(grep -E \'^GRUB_CMDLINE_LINUX=\' "$rootmnt"/etc/default/grub | head -1 || echo \'GRUB_CMDLINE_LINUX=\')
